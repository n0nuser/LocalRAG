...
Static API-key authentication does not fit FastMCP's built-in `auth=` providers, which target JWT/OAuth flows. Instead, `ApiKeyMiddleware` (a `fastmcp.server.middleware.Middleware` subclass) checks the key on every `tools/call`: over HTTP it reads `X-API-Key` via `fastmcp.server.dependencies.get_http_headers()`; over stdio, where no HTTP headers exist, it falls back to the `MCP_API_KEY` environment variable — the same fallback contract ADR 038 established. A rejected key raises `ToolError`, which both the in-process `fastmcp.Client` and a real HTTP client receive identically as `result.isError: true` with the message `"Invalid or missing API key."` — this was verified with a live `Client` and a live `uvicorn` process, not assumed from documentation.
